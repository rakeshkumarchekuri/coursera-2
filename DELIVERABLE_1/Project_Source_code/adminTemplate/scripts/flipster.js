function getflipstercontent() {
    $.get(xmlFolder + "flipster.xml")
        .success(readflipsterCompleted)
        .error(readflipsterFailed);
}


function readflipsterCompleted(data) {
    var $librarys = $(data).find('libraryType').find('libraryData');
    var $libraryTitles = $(data).find('libraryTitles').text();
    $("#librarypanelheading").append('<h4 class="heading"><b>' + $libraryTitles + '</b></h4>');

    $librarys.each(function(index, value) {

        var $libraryStatus = $(value);


        var libraImage = $libraryStatus.find('libraImage').text();
        var libraImage = imagesFolder + libraImage;

        var libraryPara = $libraryStatus.find('libraryPara').text();





        $(".flipster .flip-items").append('<li data-flip-title="Red">\
                                <img src=" ' + libraImage + ' " id="image1">\
                            </li>')
...
function readflipsterFailed() {
    var message = "Library flipster  Path is Not Found";
    ...
    $.fn.flipster = function(options) {
        ...
        if (isMethodCall) {
            ...
            return this.each(function() {
                ...
                if (methods[options]) { return methods[options].apply(this, args); } else { return this; }
            });
        }
